$(document).ready(function(){
	...
	$('input:button').button().click(function(){
		...
		$.post('modules/checkspecialtydata.php',$formdata,function(value){
			if(value == '1'){
				$.post('modules/addspecialty.php',$formdata,function(data){
					if(data == '1'){
						$message.html('Nueva especialidad registrado satisfactoriamente');
						$message.dialog({
							title:'<img alt="success" src="images/success.png" /> Registro satisfactorio',
							buttons:{
								'Ingresar Especialidad':function(){window.location = 'addspecialty.php';},
