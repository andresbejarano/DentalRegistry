...
$total = $handler->getConditionedPatientCount($where);
$body = '';
	if($total > 0){
		?>
		...
		<table class="tablesorter" style="margin-left:auto;margin-right:auto;width:85%;">
			...
			<tbody>
				<?php
				$result = $handler->getConditionedPatientList($where);
				for($i = 0;$i < $total;$i += 1){
					?>
					<tr>
						...
						<td><a href="patient.php?id=<?php echo $result[$i][0];?>">Ver Paciente</a></td>
					</tr>
					<?php
				}
				?>
